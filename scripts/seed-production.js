/**
 * Seeds real Firebase project with initial data.
...
const app = initializeApp({
  apiKey: 'AIzaSyDN64zo_bk0MtV9rC_FRHcqSElavyRepVU',
  authDomain: 'tabsii-pos.firebaseapp.com',
  projectId: 'tabsii-pos',
  storageBucket: 'tabsii-pos.firebasestorage.app',
  messagingSenderId: '584885539800',
  appId: '1:584885539800:web:d7e87ce2bb48605dde58ad',
});
...
const TABLES = [
  ...
  { id: 't2', number: 2, section: 'floor1', seats: 2, status: 'free' },
  { id: 't3', number: 3, section: 'floor1', seats: 6, status: 'free' },
  { id: 't4', number: 4, section: 'floor2', seats: 4, status: 'free' },
  { id: 't5', number: 5, section: 'floor2', seats: 4, status: 'free' },
  { id: 't6', number: 6, section: 'floor2', seats: 2, status: 'free' },
  { id: 't7', number: 7, section: 'patio', seats: 6, status: 'free' },
  { id: 't8', number: 8, section: 'patio', seats: 4, status: 'free' },
  { id: 't9', number: 9, section: 'patio', seats: 2, status: 'free' },
];
...
async function seed() {
  // Step 1: Create users (each createUser call auto signs-in as that user)
  console.log('🌱 Creating user accounts...');
  let ownerUid = null;
  for (const u of USERS) {
    try {
      const cred = await createUserWithEmailAndPassword(auth, u.email, u.password);
      // Write this user's own doc while signed in as them
      await setDoc(doc(db, 'users', cred.user.uid), {
        id: cred.user.uid, name: u.name, role: u.role, email: u.email,
      });
      if (u.role === 'owner') ownerUid = cred.user.uid;
      console.log(`  ✓ ${u.role}: ${u.email}`);
    } catch (e) {
      if (e.code === 'auth/email-already-in-use') {
        await signInWithEmailAndPassword(auth, u.email, u.password);
        console.log(`  ↺ ${u.email} already exists (signed in)`);
      } else {
  ...
  // After loop, auth.currentUser is owner (last created)
  // Step 2: Seed tables (any signed-in user can write)
  console.log('\n🌱 Seeding tables...');
  ...
  // Step 3: Seed menu items (requires owner role)
  console.log('\n🌱 Seeding menu items...');
  for (const m of MENU) {
  ...
  console.log('\n✅ Seed complete! Firebase is ready.');
  console.log('\n📱 Login credentials:');
